"""
Crear un programa que almacene el diccionario 
con los créditos de las asignaturas de un curso 
{'Matemáticas': 6, 'Física': 4, 'Química': 5} y después muestre 
por pantalla los créditos de cada asignatura en el formato <asignatura> 
tiene <créditos> créditos, donde <asignatura> es cada una de las 
asignaturas del curso, y <créditos> son sus créditos. Al final debe mostrar 
también el número total de créditos del curso.
"""

créditos_por_asignatura = {
    'Matemáticas': 6, 
    'Física': 4, 
    'Química': 5
}

total_créditos = 0

for asignatura, créditos in créditos_por_asignatura.items():
    print(f"{asignatura} tiene {créditos} créditos")
    total_créditos += créditos

print()
print(f"Créditos totales: {total_créditos}")